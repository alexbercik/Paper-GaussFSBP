...
def exponential_bases_p2(beta: float, op_type: str) -> tuple[JuliaBasis, JuliaBasis]:
    """Build p2 equivalent basis, tailoring the quad basis exactly to the available DOFs."""
    ...
    if op_type == "half-open-right":
        quad_basis = JuliaBasis(
            labels=["1", "x", f"exp({beta:g}x)", f"x exp({beta:g}x)", f"exp(2*{beta:g}x)"],
            functions=[
                "x -> one(x)", "x -> x", 
...
def get_dynamic_exp_operator(h: float, order: int, op_type: str) -> Operator:
    ...
    cache_key = f"h{h:g}_beta{beta:g}_opt_p{order}_{op_type}_v9"
    cache = load_cache()
    ...
    kwargs = {
        ...
        "digits": 64,
        "orthogonalize": True,
        "quad_kwargs": {"lost_digits": 9},
    }
    
    if op_type == "closed":
        kwargs["principal"] = "upper"  # Locks x=1, open x=0
    elif op_type == "half-open-right":
        kwargs["principal"] = "lower"  # Locks x=0, open x=1
    
    operator = build_operator_from_julia(op_basis, quad_basis, **kwargs)

    # Validates the operator class strictly back to the Python framework logic
    dataclass_op_type = "closed" if op_type in ["closed", "fully-closed"] else "open"
...
RUNS = [
    {
        "label": "LGp2 Interior + Exp p2 (Half Open Right)",
    ...
    {
        "label": "LGLp2 Interior + Exp p2 (Half Open Right)",
    ...
    {
        "label": "LGp2 Interior + Exp p2 (Fully Open Right)",
        "interior_operator": OperatorSpec("LGp2"),
        "right_op_type": "open",
        "order": 2,
        "num_right_elements": 1,
    ...
    {
        "label": "LGLp2 Interior + Exp p2 (Fully Closed Right)",
        "interior_operator": OperatorSpec("LGLp2"),
    ...
    {
        "label": "LGp3 Interior + Exp p3 (Fully Open Right)",
        "interior_operator": OperatorSpec("LGp3"),
        "right_op_type": "open",
        "order": 3,
        "num_right_elements": 1,
    },
    {
        "label": "LGLp3 Interior + Exp p3 (Fully Open Right)",
    ...
    {
        "label": "LGp3 Interior + Exp p3 (Closed Right)",
        "interior_operator": OperatorSpec("LGp3"),
        "right_op_type": "closed",
        "order": 3,
        "num_right_elements": 1,
    ...
    {
        "label": "LGLp3 Interior + Exp p3 (Closed Right)",
        "interior_operator": OperatorSpec("LGLp3"),
        "right_op_type": "closed",
        "order": 3,
        "num_right_elements": 1,
...
def count_right_elements(run: dict[str, object], num_elements: int) -> int:
    num_right_elements = run.get("num_right_elements", 0)
...
def solve_on_custom_mesh(
    ...
    """Builds a mesh where the interior is uniformly refined, but the right element is strictly fixed at h_right."""
    def left_bc_fun(_x: float) -> float:
        return float(exact_fun(DOMAIN[0]))
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("PHASE 1: STANDARD UNIFORM MESH REFINEMENT")
    print("="*60)
    ...
    for experiment in EXPERIMENTS:
        ...
        plot_convergence(
            ...
            np.vstack(err_rows),
            labels,
            title=f"{experiment['title']} (Uniform Mesh Refinement)",
            grid=True,
    ...
    print("\n\n" + "="*60)
    print("PHASE 2: FIXED BOUNDARY ELEMENT SIZE vs CONVERGENCE RATE")
    print("="*60)

    H_RIGHT_SAMPLES = [1/8, 1/16, 1/32, 1/64, 1/128, 1/256]
    INTERIOR_ELEMENT_COUNTS = [16, 32, 64, 128]

    # Isolated target configurations for Phase 2 boundary tests
    PHASE2_RUNS = [
        {
            "label": "LGLp2 Interior + Exp p2 (Fully Closed Right)",
            "interior_operator": OperatorSpec("LGLp2"),
    ...
    for experiment in EXPERIMENTS:
        print(f"\nEvaluating Fixed Boundary Convergence for: {experiment['label']}")
        ...
        for run in PHASE2_RUNS:
            ...
            for h_right in H_RIGHT_SAMPLES:
                errors = []
                hs_interior = []

                for num_interior in INTERIOR_ELEMENT_COUNTS:
                    elements, u = solve_on_custom_mesh(
                        run, num_interior, h_right,
                        exact_fun=experiment["exact_fun"],
                        f_fun=experiment["f_fun"]
                    )
                    err = global_H_error(elements, u, experiment["exact_fun"])
                    errors.append(err)
                    
                    hs_interior.append((DOMAIN[1] - DOMAIN[0] - h_right) / num_interior)

                errors_clamped = np.maximum(errors, 1e-16)
                
                # Fit the best-fit linear regression slope across the error sample points
                fit = np.polyfit(np.log(hs_interior), np.log(errors_clamped), 1)
                rate = fit[0]
                rates_for_run.append(rate)
                print(f"    h_right = {h_right:7.4f} | Interior Rate = {rate:.4f}")

            ax.plot(H_RIGHT_SAMPLES, rates_for_run, marker='o', label=run["label"])

        ax.set_xscale('log', base=2)
        ax.set_xlabel('Right Element Size ($h_{right}$)')
        ax.set_ylabel('Convergence rate')
        ax.set_title(f'Interior Convergence Rate vs Fixed Boundary Element Size\n({experiment["title"]})')
